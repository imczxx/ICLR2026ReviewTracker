...
let searchResultIds = null;  // null 表示无搜索，Set 表示搜索结果的 paper IDs
let currentSearchQuery = '';
let totalSearchResults = 0;  // 搜索结果总数
let currentSearchPage = 1;   // 当前搜索页码
let currentActiveFilter = 'all';  // 保存当前筛选状态
...
function renderPagination(currentPage, totalPages) {{
    ...
    // 上一页按钮
    if (currentPage > 1) {{
        html += `<button class="page-btn" onclick="performSearch(${{currentPage - 1}})">← Prev</button>`;
    }} else {{
    ...
    if (currentPage < totalPages) {{
        html += `<button class="page-btn" onclick="performSearch(${{currentPage + 1}})">Next →</button>`;
    }} else {{
...
function renderPaperList(papers, tabId) {{
    ...
    function renderBatch() {{
        ...
        if (currentIndex < papers.length) {{
            ...
            // 渲染完成后，恢复之前的筛选状态（仅在 active tab 且有搜索时）
